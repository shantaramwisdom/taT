# Purpose: Airflow code to orchestrate KC2 Bancs Monthly Processes.
...
with DAG(
    ...
    actuarial_kc2 = create_python_operator(
        ...
        op_kwargs={
            "emr_name": emr_name,
            "step_name": f"fKC2 Monthly Actuarial {source_system}",
    ...
    kc2_alerts = create_python_operator(
        ...
        op_kwargs={
            "emr_name": emr_name,
            "step_name": f"fKC2 Monthly Alerts {source_system}",
            "command": "sh /application/financedw/financecontrols/scripts/BancsKC6Alerts_Monthly.sh "
    ...
    kc2_monthly = create_python_operator(
        dag=dag,
        task_name="kc2_monthly",
        op_kwargs={
            "emr_name": emr_name,
            "step_name": f"fKC2 Monthly {source_system}",
            "command": "sh /application/financedw/financecontrols/scripts/bancs_kc2_monthly.sh CON"
        },
        python_callable=emr_step_submit,
        trigger_rule='all_success'
    )

    kc2_monthly_notify = create_python_operator(
        dag=dag,
        task_name="kc2_monthly_notify",
        op_kwargs={
            "emr_name": emr_name,
            "step_name": f"fKC2 Monthly Notify {source_system}",
            "command": "sh /application/financedw/financecontrols/scripts/bancs_kc2_notification_monthly.sh CON"
        },
        python_callable=emr_step_submit,
        retries=0,
        trigger_rule='all_success'
    ...
    start >> create_curated_cluster >> [actuarial_kc2, kc2_alerts] >> end
    create_curated_cluster >> kc2_monthly >> kc2_monthly_notify >> end
    ...
    if '{project_environment_name}' in ['prd']:
        ...
        trigger_emr_terminator = TriggerDagRunOperator(
            ...
            trigger_dag_id=f'TA-INDIVIDUAL-FINWD-{project_env}/{project_name}-EMR-TERMINATOR'.upper(),
            conf={
