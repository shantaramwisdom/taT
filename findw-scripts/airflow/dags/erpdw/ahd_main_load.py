...
# ----------------DEVELOPMENT LOG----------------
# 11/12/2024 - Somya Shrivastava - GLERP framework development
# 09/09/2025 - Mayank Dash - DAG Flow Enhancements for R3
...
tags=tags
 ...
 def generate_task(source_system, domain, emr_name, is_derived=False, retries=0):
  '''returns python operator for curated domain and stored procedure task group for base/derived domains'''
  hop_ovrd=''
  if domain in ['general_ledger_header','general_ledger_line_item']:
   hop_ovrd='.append'
  if not is_derived:
   with TaskGroup(group_id="curated_" + domain) as curated_tg:
    ...
    op_kwargs={
     ...
     "command": "sh /application/financedw/curated/scripts/load_curated.sh "
                ...
                f"-j {{ task_instance.xcom_pull(task_ids='start', key='job_name') }} "
                f"-r {{ task_instance.xcom_pull(task_ids='start', key='rerun_flag') }}"
     },
    python_callable=emr_step_submit,
    trigger_rule="all_success",
    retries = retries
 ...
 for domain in baseline_domain_list_findw:
  ...
  elif domain == 'general_ledger_line_item':
   curated_line_tg, findw_base_line_tg = generate_task(source_system, domain, emr_name)
  if '{project_environment_name}' not in ['prd']:
   if domain == 'contractplancodeetails':
    curated_contractplancodeetails_tg, findw_contractplancodeetails_tg = generate_task(source_system, domain, emr_name, retries=3)
   elif domain == 'contractorinsurancetreatycompidassignment':
    curated_contractorinsurancetreatycompidassignment_tg, findw_contractorinsurancetreatycompidassignment_tg = generate_task(source_system, domain, emr_name)
 if '{project_environment_name}' not in ['prd']:
  start >> create_curated_cluster >> [curated_contractplancodeetails_tg >> [curated_contractorinsurancetreatycompidassignment_tg, findw_contractplancodeetails_tg, findw_contractorinsurancetreatycompidassignment_tg],
  curated_header_tg >> curated_line_tg >> (end_curated, findw_base_header_tg, findw_base_line_tg) >> end_findw]
 else :
