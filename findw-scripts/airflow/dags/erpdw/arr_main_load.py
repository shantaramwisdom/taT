...
arr_domain = 'erpdata_approval_view'
...
with DAG(
    ...
    trigger_emr_terminator = TriggerDagRunOperator(
        ...
        trigger_dag_id=f'TA-INDIVIDUAL-FINDW-{project_environment_name}-{project_name}-ERPDW-EMR-TERMINATOR'.upper(),
        conf={
            ...
            with TaskGroup(group_id="curated_" + domain) as curated_tg:
                curated_currentbatch = create_python_operator(
                    ...
                    op_kwargs={
                        ...
                        "command": "sh /application/financedw/curated/scripts/load_curated.sh "
                                   f"-s {source_system} -d currentbatch_{domain} -f {batch_frequency.upper()} -g A "
                ...
                curated_completedbatch = create_python_operator(
                    ...
                    op_kwargs={
                        ...
                        "source_system_name": emr_source_system,
                        "command": "sh /application/financedw/curated/scripts/load_curated.sh "
                                   f"-s {source_system} -d completedbatch_{domain} -f {batch_frequency.upper()} -g A "
                                   "-j {{ task_instance.xcom_pull(task_ids='start', key='job_name') }}"
                    },
                    python_callable=emr_step_submit,
                    trigger_rule="all_success"
...
for domain in baseline_domain_list_findw:
    if domain == 'general_ledger_header_contractnumberlevelreserves':
        curated_reservesheader_tg, findw_reservesheader_tg = generate_task(source_system, domain, emr_name)
    elif domain == 'general_ledger_line_item_contractnumberlevelreserves':
        curated_reservesline_tg, findw_reservesline_tg = generate_task(source_system, domain, emr_name)
    elif domain == 'general_ledger_header':
        curated_header_tg, findw_header_tg = generate_task(source_system, domain, emr_name)
    elif domain == 'general_ledger_line_item':
        curated_line_tg, findw_line_tg = generate_task(source_system, domain, emr_name)
start >> branch_object_check_task >> create_curated_cluster >> notify_skipped_load
create_curated_cluster >> curated_line_tg >> [findw_reservesheader_tg, findw_reservesline_tg]
curated_header_tg >> curated_line_tg >> [end_curated, end_findw]
[findw_reservesheader_tg, findw_reservesline_tg, findw_header_tg, findw_line_tg] >> end_findw