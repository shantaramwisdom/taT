...
target_data_temp as (
    select python_date_format_checker(transaction_date, '%Y%') as transaction_date_chk,
           python_date_format_checker(gl_reversal_date, '%Y%', True) as gl_reversal_date_chk,
           gl_source_code,
    ...
    from source_data a
        left join lkp_actvty_gl_sbldgr_nm b on a.gl_application_area_code = b.ACTVITY_GL_APP_AREA_CD
        and substr(gl_source_code, 1, 3) = b.ACTVITY_GL_SRC_CD
        left join lkp_actvty_ldgr_nm o on substr(secondary_ledger_code, 3, 2) = c.actvty_gl_ldgr_cd
        ...
        and a.orig_gl_center = d.gac_centers
        and substr(gl_full_source_code, 1, 3) in ('903', '851', '444')
...
target_force_ignore as (
    ...
    where transaction_date_chk = 'true'
      ...
      and substr(secondary_ledger_code, 3, 2) not like 'x%'
...
new_errors as (
    ...
    from (
        ...
        where transaction_date_chk = 'false'
           ...
           or substr(gl_source_code, 1, 3) like 'x%'
           or substr(secondary_ledger_code, 3, 2) like 'x%'
